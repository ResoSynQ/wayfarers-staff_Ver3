/**
 * 旅人の杖と救いの泉 Ver 2.0.23 (全国熱気レーダー対応版)
...
function renderGeoJson(key, bounds = null) {
    ...
    L.geoJSON(rawData[key], {
        ...
        pointToLayer: function(feature, latlng) {
            ...
            // ✨【魔法の救済処理】観光資源(kanko)等の面データに「点」が混ざっていた場合、見えなくなるのを防ぐ！
            if (def.style && !def.icon) {
                let markerColor = '#3388ff'; // デフォルト青
                if (typeof def.style === 'function') {
                    const evalStyle = def.style(feature);
                    if (evalStyle && evalStyle.color) markerColor = evalStyle.color;
                } else if (def.style.color) {
                    markerColor = def.style.color;
                }
                // 設定された色を使って、丸いマーカー(circleMarker)として超高速描画する
                return L.circleMarker(latlng, {
                    radius: 5,
